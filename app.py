        ...
        try:
            if " - " in el:
                ...
                if data_ricorrenza < oggi - timedelta(days=1):
                    data_ricorrenza = datetime(anno_corrente + 1, mese, girono)
                    
                differenza = (data_ricorrenza - ogggi).days
                if 0 <= differenza <= 7:
                    alert_trovati.append({"nome": nome, "giorni": differenza, "data": data_str})
        except:
            continue
    return alert_trovati
# ===== BLOCCO 5: SCANNER INTERFACCIA ALERT =====
...
if not df.empty:
    for idx, row in df.iterrows():
        note_campo = row["Note aggiuntiv e"]
...
# ===== BLOCCO 7: SCHEDA DI MODIFICA =====
    st.markdown("### 📝 Scheda Anagrafica e Stato Prenotazione")
    
    note_intere = str(riga_selezionata["Note aggiuntiv e"])
    compleanni_correnti = ""
    onomastici_correnti = ""
    ...
    with st.form("modulo_modifica_ospite"):
        ...
        with c1:
            ...
            mod_arrivo = st.text_input("Data di Arrivo (GG/MM/AAAA)", value=st.session_state.get("rx_arr", str(riga_selezionata["data presunta di Arriv o"])))
            mod_adulti = st.text_input("Numero Adulti", value=st.session_state.get("rx_ad", str(riga_selezionata["adult i"])))
        with c2:
            mod_nome = st.text_input("Nome", value=str(riga_selezionata["Nom e"]))
            mod_partenza = st.text_input("Data di Partenza (GG/MM/AAAA)", value=st.session_state.get("rx_part", str(riga_selezionata["data presunta di Partenz a"])))
            mod_minori = st.text_input("Numero Minori", value=st.session_state.get("rx_min", str(riga_selezionata["minor i"])))
        with c3:
            mod_email = st.text_input("Email", value=st.session_state.get("rx_em", str(riga_selezionata["Emai l"])))
            mod_ospiti_tot = st.text_input("Totale Componenti Gruppo", value=st.session_state.get("rx_tot", str(riga_selezionata["Numero Ospit i"])))
            mod_esito = st.selectbox("Stato Pratica (Esito)", ["🔄 In corso", "🔄 In sospeso", "🔄 Pre-approvata", "✅ Confermata", "📋 Lista attesa", "❌ Non disponibile", "❌ Cancellata", "❌ Annullata"], index=["🔄 In corso", "🔄 In sospeso", "🔄 Pre-approvata", "✅ Confermata", "📋 Lista attesa", "❌ Non disponibile", "❌ Cancellata", "❌ Annullata"].index(str(riga_selezionata["Esit o"])) if str(riga_selezionata["Esit o"]) in ["🔄 In corso", "🔄 In sospeso", "🔄 Pre-approvata", "✅ Confermata", "📋 Lista attesa", "❌ Non disponibile", "❌ Cancellata", "❌ Annullata"] else 0)
...
# ===== BLOCCO 8: MOTORE SCRITTURA CSV =====
    if salva_premuto:
        ...
        df.at[idx_riga, "Cognom e"] = mod_cognome
        df.at[idx_riga, "Nom e"] = mod_nome
        df.at[idx_riga, "Emai l"] = mod_email
        df.at[idx_riga, "data presunta di Arriv o"] = mod_arrivo
        df.at[idx_riga, "data presunta di Partenz a"] = mod_partenza
        df.at[idx_riga, "Numero Ospit i"] = mod_ospiti_tot
        df.at[idx_riga, "adult i"] = mod_adulti
        df.at[idx_riga, "minor i"] = mod_minori
        df.at[idx_riga, "Portale di provenienz a"] = mod_portale
        df.at[idx_riga, "Cane (Razza/Taglia)"] = mod_cane
        df.at[idx_riga, "Esit o"] = mod_esito
        df.at[idx_riga, "Note aggiuntiv e"] = nota_unificata
...
# ===== BLOCCO 9: MODULO COMUNICAZIONE =====
    ...
    if scelta_modello == "Modello Tutto Esaurito (15% Sconto)":
        ...
        testo_finale_comunicazione = (
            f"Ciao {nome_titolare}, ti ringraziamo molto per haber pensato alla nostra struttura per le tue vacanze! "
    ...
    if scelta_modello != "Nessuno":
        ...
        link_whatsapp = f"https://wa.me{testo_url_encoded}"
